/*
 * asERP storefront tracker (canonical core).
 *
 * Privacy & security model:
 *  - Sends beacons to a SAME-ORIGIN collector path on the shop (configured by the plugin). The shop
 *    plugin proxies them server-side to asERP, adding the secret token + visitor IP + (when logged in)
 ...
 *
 * Configure before loading by setting window.aserpTrackConfig = { endpoint: '/same-origin/path',
 * product: { ref, name, value, currency } }. The plugin injects this.
 */
(function () {
  "use strict";

  var cfg = window.aserpTrackConfig || {};
