...
/**
 * Same-origin collector that proxies storefront beacons to asERP. The secret tracking token is read from
 * the plugin config and added here — it never reaches the browser. When a customer is logged in, a
 * pseudonymised reference (cid) is computed and forwarded so asERP can attribute product views.
 */
...
class CollectorController extends StorefrontController
{
    ...
    #[Route(
        path: '/aserp-analytics/c',
        name: 'frontend.aserp.collect',
        methods: ['POST'],
    ...
    /**
     * Fire-and-forget forward to asERP with a tight timeout, so the beacon response is never blocked on
     * the upstream call.
     */
