 ...
 * Requires PHP:      7.4
 * Text Domain:       aserp-analytics
 *
 * Security model:
 *  - The browser only ever talks to THIS shop (same-origin REST route). The secret token is added here,
 *    server-side, and forwarded to asERP — it is never exposed in page source or network traffic.
 *  - When a customer is logged in, a pseudonymised reference (cid = HMAC of the customer id, keyed by a
 *    value derived from the token) is computed here and forwarded, so asERP can attribute product views
...
{
    const OPT_SERVER_URL = 'aserp_analytics_server_url';
    const OPT_TOKEN      = 'aserp_analytics_token';

    const REST_NAMESPACE = 'aserp/v1';
    const REST_ROUTE     = '/c'; // first-party collector: /wp-json/aserp/v1/c
    ...
    {
        add_options_page(
            __('asERP Web Analytics', 'aserp-analytics'),
            __('asERP Analytics', 'aserp-analytics'),
            'manage_options',
            'aserp-analytics',
    ...
    {
        register_setting('aserp_analytics', self::OPT_SERVER_URL, [
            'type'              => 'string',
            'sanitize_callback' => [__CLASS__, 'sanitize_url'],
            'default'           => '',
        ]);
        register_setting('aserp_analytics', self::OPT_TOKEN, [
            'type'              => 'string',
    ...
    public static function render_settings_page()
    {
        ...
        ?>
        <div class="wrap">
            <h1><?php esc_html_e('asERP Web Analytics', 'aserp-analytics'); ?></h1>
            <p><?php esc_html_e('Connect this store to your asERP server. Generate a tracking token in asERP (Sales channel → Web analytics) and paste it here.', 'aserp-analytics'); ?></p>
            <form action="options.php" method="post">
                <?php settings_fields('aserp_analytics'); ?>
                <table class="form-table" role="presentation">
                    <tr>
                        <th scope="row"><label for="aserp_url"><?php esc_html_e('asERP Server URL', 'aserp-analytics'); ?></label></th>
                        <td>
                            <input name="<?php echo esc_attr(self::OPT_SERVER_URL); ?>" id="aserp_url" type="url"
                                   class="regular-text" placeholder="https://erp.example.com"
                                   value="<?php echo esc_attr(get_option(self::OPT_SERVER_URL, '')); ?>" />
                        </td>
                    </tr>
                    <tr>
                        <th scope="row"><label for="aserp_token"><?php esc_html_e('Tracking Token', 'aserp-analytics'); ?></label></th>
                        <td>
                            <input name="<?php echo esc_attr(self::OPT_TOKEN); ?>" id="aserp_token" type="password"
                                   class="regular-text" autocomplete="off"
                                   value="<?php echo esc_attr(get_option(self::OPT_TOKEN, '')); ?>" />
                            <p class="description"><?php esc_html_e('Stored on the server only — never sent to the browser.', 'aserp-analytics'); ?></p>
                        </td>
                    </tr>
                </table>
                <?php submit_button(); ?>
    ...
    public static function enqueue_tracker()
    {
        ...
        $version = '1.0.0';
        $core_url = plugins_url('assets/aserp-s.js', __FILE__);
        $glue_url = plugins_url('assets/aserp-wc.js', __FILE__);

        wp_register_script('aserp-analytics-core', $core_url, [], $version, true);
        wp_register_script('aserp-analytics-wc', $glue_url, ['aserp-analytics-core', 'jquery'], $version, true);
        ...
        wp_localize_script('aserp-analytics-core', 'aserpTrackConfig', $config);
        wp_enqueue_script('aserp-analytics-core');
        wp_enqueue_script('aserp-analytics-wc');
    }

    // -------------------------------------------------------------------------
    // Same-origin collector → server-side proxy to asERP
    // -------------------------------------------------------------------------
