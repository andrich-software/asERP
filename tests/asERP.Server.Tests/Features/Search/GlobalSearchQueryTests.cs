...
namespace asERP.Server.Tests.Features.Search;
...
public class GlobalSearchQueryTests : TenantIsolatedTestBase
{
    private async Task SeedSearchTestDataAsync()
    {
        ...
        DbContext.Customer.AddRange(
            new asERP.Domain.Entities.Customer
            {
        ...
        DbContext.Invoice.Add(new asERP.Domain.Entities.Invoice
        {
    ...
    private async Task<(asERP.Domain.Entities.Shipping Tenant1Shipping,
        asERP.Domain.Entities.Shipping Tenant2Shipping)> SeedShippingSearchDataAsync()
    {
        await TestDataSeeder.SeedTestDataAsync(DbContext, TenantContext);
