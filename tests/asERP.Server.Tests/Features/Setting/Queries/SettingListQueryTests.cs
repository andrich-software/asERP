...
namespace asERP.Server.Tests.Features.Setting.Queries;
...
public class SettingListQueryTests : GlobalTestBase
{
    ...
    private async Task SeedSettingTestDataAsync()
    {
        ...
        if (!existingSettings.Any(s => s.Key == "test.setting.global"))
        {
            ...
            var setting3 = new asERP.Domain.Entities.Setting
            {
                Key = "test.theme.global",
                Value = "dark"
            };

            var setting4 = new asERP.Domain.Entities.Setting
            {
                Key = "test.config.global",
                Value = "config_value"
            };

            var setting5 = new asERP.Domain.Entities.Setting
            {
