# CLAUDE.md — asERP.Server.Tests

xUnit integration tests against `asERP.Server`, with first-class multi-tenant support.
...
Location: `tests/asERP.Server.Tests/Infrastructure/TenantIsolatedTestBase.cs`.
...
Location: `tests/asERP.Server.Tests/Infrastructure/TestAssertions.cs`. Thin wrappers around xUnit for common needs (HTTP status, nullability, collections).
