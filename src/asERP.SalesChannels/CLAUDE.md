...
- Named HttpClients per channel with `.AddPollyHandlers()` (retry + circuit breaker) and `.AddSsrfGuardedPrimaryHandler()` (ConnectCallback re-validates dialed IPs via `SalesChannelUrlValidator` against DNS-rebind/redirect SSRF). Exception: `"amazon-lwa"` (fixed Amazon host). WooCommerce REST runs on its named client too (`"woocommerce"`, guard only, no Polly): WooCommerceNET 0.8.7 sends through `HttpWebRequest` and takes no handler, so `GuardedRestApi` overrides the SDK's virtual `SendHttpClientRequest` and puts the SDK-built request on that client.
- **Any tenant-controlled URL/host (channel URL, image URL, MySQL host) must pass `SalesChannelUrlValidator`.** Exceptions to it are operator-owned: `SalesChannelHostPolicy` (section `SalesChannelHostPolicy`, allowed private CIDRs + the MySQL TLS settings: `SslCaPath`, `AllowCertificateHostnameMismatch`, and `AllowInsecureTransport` as the only route out of a verifying `SslMode`), injected — never a flag out of `AdditionalConfigJson`, which the same caller supplies. Such keys are stripped from every tenant path via `SalesChannelConfigOperatorKeys`.
- **The direct-MySQL connector defaults to `SslMode.VerifyFull`** — `Required` authenticates nobody, and its forged certificate even passes as a verified identity, so a MitM offering `mysql_clear_password` gets the shop DB password in cleartext. Breaking for a channel whose server certificate is self-signed or issued for another name; ladder, in this order: publicly trusted CA for the configured host (nothing to set) → private CA: `SslCaPath` at its PEM bundle, stays `VerifyFull` → wrong name: `AllowCertificateHostnameMismatch` (`VerifyCA` — chain still verified, but no name check **and no revocation check**) → no TLS at all: `AllowInsecureTransport`. A revocation endpoint the server cannot reach is a different failure and belongs to `SkipCertificateRevocationCheck` (not exposed; see the `SslCaPath` doc), never to the last two rungs. The connect-failure text names all three keys; the server log holds the exception and the attempted mode.
...
- **A transport failure must not describe the network to the caller.** Connectors catch `Exception` and pass `ex.Message` on almost everywhere, and that text becomes `ChannelSyncRun.ErrorSummary` (four `[Authorize]`-only readers) or `ChannelExportOutbox.LastError` (two more), plus `ChannelSyncLog.Message`/`.Exception` via `GET sync-logs`. So whoever *dials* classifies: a failure describing reachability is wrapped in `ChannelTransportException`, whose `Message` is the caller-safe constant and whose `InnerException` is the operator's. `SalesChannelSyncLogSink` drops any event marked that way. Log the *wrapper*, never the bare provider exception. A remote system that answered and rejected the data (`HTTP {status}: {body}`, a bad SKU, a missing category) is the diagnostic the sync-log view exists for and stays verbatim.
  - **The sink's limit:** it classifies on `logEvent.Exception` alone and cannot see what a caller interpolated into the message template. Never format a reason into a log message — attach the exception. `SyncLogSinkReachTests` pins that a line logged by any category under the dispatcher's scope is captured.
  - Wrapped today: `WooCommerceDatabaseConnector.OpenAsync` (MySQL connect), `SalesChannelUrlValidator`'s resolution-dependent rejections (covering all 16 `Validate` call sites at once), and the SSRF guard's blocked-address rejection in `SalesChannelServiceRegistration`. **Not** wrapped: the HTTP connectors' own connect failures — refused, timed out and TLS still reach the caller as `ex.Message`, and a guard rejection is still tellable from an ordinary failure. `SyncDispatcher`'s catch-all backstop passes `ex.Message` through unclassified: that is the boundary of the design, not a covered case.
  - Diagnosis afterwards: a run's `CorrelationId` is the server log's `SyncRunCorrelationId`. Only `GET sync-runs` and `GET sync-runs/{runId}` carry it — `SyncOperationStatusDto` (sync-status) and `SalesChannelSyncResultDto` (the 202 body) have no such field, so a user who saw the error there must pivot through `GET sync-runs` to name the run.
  - `CategorySalesChannel.LastErrorMessage` and `ProductSalesChannel.LastErrorMessage` are written from the same unsanitised `ExportResult.Fail(ex.Message)` (`SyncDispatcher` ~912/962) and have **no reader today**. Adding a DTO field for either opens this surface again — sanitise at the dial site first.
