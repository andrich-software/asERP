...
/// Implements IRequestHandler from the custom mediator to handle TenantDetailQuery requests
...
public class TenantDetailHandler : IRequestHandler<TenantDetailQuery, Result<TenantDetailDto>>
{
    ...
    public async Task<Result<TenantDetailDto>> Handle(TenantDetailQuery request, CancellationToken cancellationToken)
    {
        ...
        if (!hasAccess)
        {
            ...
            return Result<TenantDetailDto>.Forbidden(ErrorCodes.Tenant.Forbidden,
        ...
        if (tenant == null)
        {
            ...
            return Result<TenantDetailDto>.NotFound(ErrorCodes.Tenant.NotFound,
