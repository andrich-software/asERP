...
public class ReturnCreateHandler : IRequestHandler<ReturnCreateCommand, Result<Guid>>
{
    ...
    public async Task<Result<Guid>> Handle(ReturnCreateCommand request, CancellationToken cancellationToken)
    {
        ...
        if (sales == null)
        {
            return Result<Guid>.Invalid(ErrorCodes.Returns.Invalid, "The sales order does not exist.");
        }

        if (sales.Status == SalesStatus.Cancelled)
        {
            return Result<Guid>.Invalid(ErrorCodes.Returns.Invalid, "A cancelled order cannot be returned.");
        }
        ...
        if (request.RequestLabel)
        {
            ...
            if (provider == null)
            {
                return Result<Guid>.Invalid(ErrorCodes.Returns.Invalid, "The shipping provider does not exist.");
            }

            if (!provider.IsEnabled)
            {
                return Result<Guid>.Invalid(ErrorCodes.Returns.Invalid,
