...
namespace asERP.Application.Features.TenantOAuthAppSettings.Commands.TenantOAuthAppSettingsUpsert;
    ...
    : IRequestHandler<TenantOAuthAppSettingsUpsertCommand, Result<Guid>>
{
        ...
        TenantOAuthAppSettingsUpsertCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            return Result<Guid>.Invalid(ErrorCodes.TenantOauthAppSettings.Invalid, "No active tenant in context.");
        }

        if (request.Provider is not (SalesChannelType.eBay or SalesChannelType.Amazon))
        {
            return Result<Guid>.Invalid(ErrorCodes.TenantOauthAppSettings.Invalid,
        ...
        if (existing is null)
        {
            ...
            return new Result<Guid> { Succeeded = true, Data = entity.Id, Status = ResultStatus.Created };
        }
