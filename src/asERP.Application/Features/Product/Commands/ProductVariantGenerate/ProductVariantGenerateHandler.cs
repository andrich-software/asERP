...
public class ProductVariantGenerateHandler : IRequestHandler<ProductVariantGenerateCommand, Result<List<Guid>>>
{
    ...
    public async Task<Result<List<Guid>>> Handle(ProductVariantGenerateCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            return Result<List<Guid>>.Invalid(ErrorCodes.Product.Invalid,
        ...
        if (parent == null)
        {
            return Result<List<Guid>>.NotFound(ErrorCodes.Product.NotFound, "Parent product not found.");
        }

        if (parent.ProductType != ProductType.VariantParent)
        {
            return Result<List<Guid>>.Invalid(ErrorCodes.Product.Invalid, "Parent product is not a variant parent.");
        }

        var axes = parent.VariantAxes.OrderBy(a => a.SortOrder).ToList();
        if (axes.Count == 0)
        {
            return Result<List<Guid>>.Invalid(ErrorCodes.Product.Invalid, "Parent product has no variant axes defined.");
        }
