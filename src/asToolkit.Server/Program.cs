...
// Out-of-band CLI mode: `dotnet asToolkit.Server.dll cli ...` runs an admin task
// against the configured database and exits without bringing up Kestrel.
if (args.Length > 0 && args[0] == "cli")
{
    return await asToolkit.Server.Cli.CliRunner.RunAsync(args[1..]);
}
...
if (grafanaSettings.LogsEnabled && Uri.TryCreate(grafanaSettings.LokiEndpoint, UriKind.Absolute, out _))
{
    builder.Host.UseSerilog((context, services, configuration) => configuration
        ...
        .WriteTo.GrafanaLoki(
            ...
            labels: new[]
            {
                new LokiLabel { Key = "app", Value = "asToolkit.Server" },
...
builder.Services.AddGrafanaTelemetryServices(grafanaSettings, "asToolkit.Server");
...
app.UseMiddleware<asToolkit.Server.Middleware.TenantMiddleware>(); // set tenant context
...
if (app.Environment.IsDevelopment())
{
    ...
    app.UseSwaggerUI(c =>
    {
        c.SwaggerEndpoint("/swagger/v1/swagger.json", "asToolkit.Server v1");
    });
}
...
// ASTOOLKIT_PUBLIC_PORT is set (e.g. by docker-compose), it overrides the port
...
{
    var publicPort = Environment.GetEnvironmentVariable("ASTOOLKIT_PUBLIC_PORT");
...
Console.WriteLine();
Console.WriteLine("========================================");
Console.WriteLine("          asToolkit Server Started         ");
Console.WriteLine("========================================");
...
Console.WriteLine("Free Web-UI: https://www.astoolkit.de/");
Console.WriteLine("========================================");
Console.WriteLine();
