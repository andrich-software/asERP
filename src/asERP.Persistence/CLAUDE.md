...
Zero-based pagination, implemented in `QueryableExtensions.cs` (in `asERP.Application` or this layer — see the file). New endpoints **must** use it; do not roll your own.
...
Persistence-level tests live in `tests/asERP.Persistence.Tests` (xUnit, EF Core InMemory). Integration tests that hit the API surface live in `tests/asERP.Server.Tests` — see its `CLAUDE.md`.
