...
namespace maERP.Client.Features.Auth.Views;

/// <summary>
/// Programmatically built dialog for managing the local list of maERP servers
...
public sealed class ServerManagementDialog
{
    ...
    private async Task UpdateRowStatusAsync(ServerRow row)
    {
        ...
        if (info != null)
        {
            row.Status.Text = string.Format(L("ServerDialog.StatusConnectedFormat", "maERP v{0} · verbunden"), info.Version);
    ...
    private async Task TestConnectionAsync()
    {
        ...
        if (info != null)
        {
            _testResult.Text = string.Format(L("ServerDialog.TestSuccessFormat", "Verbindung erfolgreich · maERP v{0}"), info.Version);
