...
The active Uno features are declared in `asERP.Client.csproj` under `<UnoFeatures>` — bumping/changing them is the canonical way to enable new capabilities.
...
Custom controls live in `Controls/` (namespace `asERP.Client.Controls`, xmlns `controls`). **Use these instead of hand-building cards, badges, field blocks or empty states:**
