...
namespace asERP.Client.Features.Auth.Views;

/// <summary>
/// Programmatically built dialog for managing the local list of asERP servers
...
public sealed class ServerManagementDialog
{
    ...
    private async Task UpdateRowStatusAsync(ServerRow row)
    {
        ...
        if (info != null)
        {
            row.Status.Text = string.Format(L("ServerDialog.StatusConnectedFormat", "asERP v{0} · verbunden"), info.Version);
    ...
    private async Task TestConnectionAsync()
    {
        ...
        if (info != null)
        {
            _testResult.Text = string.Format(L("ServerDialog.TestSuccessFormat", "Verbindung erfolgreich · asERP v{0}"), info.Version);
