# CLAUDE.md — asERP.Server

ASP.NET Core Web API (net10.0). Headless backend; consumed by `asERP.Client` and any HTTP client.
...
Rules:
- **Primary-constructor injection** of `IMediator` (the custom one from `asERP.Application.Mediator`).
...
## CQRS in asERP.Application

Commands and queries live in `src/asERP.Application/Features/{Area}/{Commands|Queries}/{Name}/`. Each folder typically contains:
...
| Task | Where |
|---|---|
| Add a CRUD resource | New `Features/{Area}/...` folder + `{Area}Controller.cs` + DTOs in `asERP.Domain/Dtos/{Area}` |
| New auth role | `asERP.Identity` constants + role check in handler + tests |
...
Server integration tests live in `tests/asERP.Server.Tests` (xUnit). All multi-tenant tests inherit `TenantIsolatedTestBase`. See that project's `CLAUDE.md`.
