...
- SMTP uses **MailKit**; Microsoft 365 uses Graph app-only (`ClientSecretCredential`, `Mail.Send` application permission).
- **The SMTP transport is the operator's decision, never the tenant's** — `SmtpEndpointGuard.ResolveTransportAsync`, called by `SmtpEmailProvider` before `ConnectAsync`: port 465 → `SslOnConnect` (implicit TLS, RFC 8314), every other port → `StartTls`, which **fails closed** where the old `Auto` continued in the clear against a server that drops the STARTTLS capability. `SecureSocketOptions.None` survives for two endpoints only, and both need `SmtpEnableSsl == false` **plus `SmtpEnableSslIsOperatorConfigured`** (third provenance mark, same rule as host/port: the tenant row left the field null): the operator's own relay on **loopback** (all three marks and a host resolving to loopback and nothing else) and `SmtpHostPolicy:AllowInsecureTransport`, the operator's installation-wide switch for a LAN relay with no TLS (mirrors `SalesChannelHostPolicy:AllowInsecureTransport`). **Keep the loopback carve-out** — without it the documented answer for a Mailpit developer is that switch, which permits cleartext to *any* admitted host, while the carve-out confines the same convenience to a session that never leaves the machine. `SmtpEnableSsl` stays in `TenantEmailSettingsInputDto` (no contract change) but can only ever *ask* for cleartext where the operator already permits it; a tenant row setting it selects nothing, anywhere.
- **A fresh install sends no mail, and Mailpit needs three settings.** The migration seeds `Email.SmtpHost = ""`, `SmtpPort = "587"`, `SmtpEnableSsl = "true"` (`SettingsSeeder`, `InitDb`); `SettingsInitializer` only adds *missing* keys and runs after `Migrate()`, so its `localhost`/`1025`/`False` values never reach a migrated database. For the Mailpit of `docker-compose.mail.yml` set **`Email.SmtpHost=localhost`, `Email.SmtpPort=1025` and `Email.SmtpEnableSsl=False`** in the Superadmin settings: leaving the seeded `true` selects `StartTls`, Mailpit offers no STARTTLS, and the send fails log-only like every refusal here. F3's rule — never authenticate over an unencrypted connection — is closed **except for the operator's own opt-ins**: AUTH still goes out in the clear on that loopback relay, and wherever `AllowInsecureTransport` is set it carries the *server's* credentials for a tenant that echoes the operator's host (`MergeWithTenant` inheritance).
