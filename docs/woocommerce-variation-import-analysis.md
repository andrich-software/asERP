# Analysis: Importing WooCommerce Product Variations into maERP
...
| | WooCommerce source (`production`, MariaDB `diy-stoffe_production`) | ERP (`maerp_01`, Postgres) |
|---|---|---|
...
### Connector — `src/maERP.SalesChannels/Connectors/WooCommerce/WooCommerceConnector.cs`
...
### Import repository — `src/maERP.SalesChannels/Repositories/ProductImportRepository.cs`
...
### Import model — `src/maERP.SalesChannels/Models/SalesChannelImportProduct.cs`
...
- **rider** SQL bridge (`mcp__rider__execute_sql_query`) returns "not done" — broken in this environment; use the maERP REST API for ERP reads instead (login `admin@localhost.com` / `P@ssword1`, send `X-Tenant-Id: d97a2d78-d929-4c4a-a823-d117ed0ec1a3`, base `https://localhost:8443`).
- **phpstorm** SQL works for the WooCommerce source DB (project `/Users/martin/Projekte/andrich-software/diy-stoffe.de`).
- The dev server runs Development config → Postgres `maerp_01` on `localhost:5432` (rider "Testsystem"). Rider "Produktivsystem" (`localhost:5434/maerp`) was offline during analysis.
